...
public class KonosubaRoutine {
    ...
    KonosubaRoutine(GameLibrary20 gl, AutoJobEventListener el) {
        mGL = gl;
        mCallbacks = el;

        String resolution = mGL.getDeviceResolution()[0] + "x" + mGL.getDeviceResolution()[1];
        mDef = DefinitionLoader.getInstance().requestDefData(R.raw.konosuba_definitions, "konosuba_definitions.xml", resolution);
